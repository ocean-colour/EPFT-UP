...
> **Provenance.** Imported from IOPtics `moana` @ `3aa3b6e` on 2026-09-13; original path `reports/MOANA_Claude_Report.md`.
...
`epft_up/moana/` (299-test suite green); the §7.1 PC-mapping dispute is
...
| `pca_picophyto.h5` + `picophyt.json` (`epft_up/data/moana/`) | the PCA loadings and coefficients — published nowhere in the literature |
...
> (`epft_up.moana.validation.bitexact_pace`, 100k ocean pixels of the
...
Per the decisions recorded in `claude_prompts/moana_IOPtics_history.md`:
...
Everything in this section is first-hand, produced by `epft_up/moana/`
...
in the prompt-12…15 log entries of `claude_prompts/moana_IOPtics_history.md`.
...
**`epft_up/data/moana/pca_picophyto.h5`** (24,488 bytes, sha256 `478ace87…deb57`)
...
**`epft_up/data/moana/picophyt.json`** (390 bytes, sha256 `cd6b5c1c…756d4`)
